              ...
              <div class="item">
                ...
                <ul>
                  <li v-for="(list, index3) in topic[3]" :key="index3">
                    <a href="javascript:;" @click="judge(index3)" :class="{'border': index == index3 && currentType == 3,'bg': bg_flag && topic[3][index3].isClick == true}"><span :class="{'mark': topic[3][index3].isMark == true}"></span>{{topicCount[0]+index3+1}}</a>
                  </li>
                </ul>
              </div>
              <div class="item">
        ...
        <div class="right">
          ...
          <div class="content">
            ...
            <div v-if="currentType == 1">
              <el-radio-group v-model="radio[index]" @change="getChangeValue" >
              ...
              <!-- <a-radio-group v-model="radio[index]" @change="getChangeValue">
                <a-radio :value="1">A. {{showAnswer.answerA}}</a-radio>
                <a-radio :value="2">B. {{showAnswer.answerB}}</a-radio>
                <a-radio :value="3">C. {{showAnswer.answerC}}</a-radio>
                <a-radio :value="4">D. {{showAnswer.answerD}}</a-radio>
              </a-radio-group> -->
            ...
            <div class="judge" v-if="currentType == 3">
              ...
              <!-- <a-radio-group v-model="judgeAnswer[index]" @change="getJudgeValue" v-if="currentType == 3">
...
export default {
  ...
  methods: {
    ...
    getExamData() { //获取当前试卷所有信息
      let date = new Date()
      this.startTime = this.getMyTime(date)
      let examCode = this.$route.query.examCode //获取路由传递过来的试卷编号
      request({
        url: '/exam/getExamInfoByExamCode',      //通过examCode请求试卷详细信息
        method: 'get',
        params: {
          examCode: examCode
        }
      }).then(res => {
            this.examData = res.data.data[0];
            this.index = 0;
            this.time = this.examData.totalTime;
            let paperId = this.examData.paperId;
            request({
              url: '/paper/getPaperById',     //通过paperId获取试题题目信息
              method: 'get',
              params: {
                paperId: paperId,
              }
              ...
              keys.forEach(element =>{
                ...
                for (let index = 0; index < data.length; index++) {
                  currentScore += Number(data[index].score);    //循环每种题型,计算出总分
                }
              ...
              let father = [];  
              for (let index = 0; index < len; index++) {    //根据填空题数量创建二维空数组存放每道题答案
                let children = [null,null,null,null];
                father.push(children);
              }
              console.log("父数组",father)
              this.fillAnswer = father
              console.log("填空题答案",this.fillAnswer)
    ...
    getChangeValue(val) { //获取选择题作答选项
    console.log("被选中选项",val);
    ...
    getJudgeValue(val) {  //获取判断题作答选项
      console.log("判断题选中",val);
    ...
    mark() { //标记功能
      switch(this.currentType) {
        ...
        case 4:
          this.topic[4][this.index]["isMark"] = true //问答题标记
      }
    },
    commit() { //答案提交计算分数
      /* 计算选择题总分 */
      ...
      console.log("最终填空题答案",this.fillAnswer)
      console.log("最终问答题答案",this.essayAnswer)
      /** 把各问答题Id以及学生填写的答案收集起来 */
      let essayQuestions = [];
      let eqs = this.topic[4];
      let i = 0;
      eqs.forEach(e => {
        let essayQuestion = {
          questionId: "",
      ...
      console.log("要提交的问答题答案",essayQuestions)
      /**将问答题推送至教师处 */
      if(this.time != 0) {
