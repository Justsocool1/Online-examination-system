...
export default {
  ...
  methods: {
    ...
    request({
        url: `/exam/getExams/${this.pagination.current}/${this.pagination.size}/${this.examName}`
    }).then((res) => {
    ...
    add(paperId,source) { //增加题库
      this.$router.push({path:'/index/addAnswerChild',query: {paperId: paperId,subject:source}})
    }
  },
};
</script>
