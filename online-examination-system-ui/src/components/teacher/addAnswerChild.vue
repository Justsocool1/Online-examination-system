...
<template>
  <div class="add">
    ...
    <el-tab-pane name="first">
      ...
      <span><i class="el-icon-circle-plus"></i>添加试题</span>
        </template>
      <section class="append">
        <ul>
          ...
          <li v-if="optionValue == '问答题'">
            <span>所属章节：</span>
            <el-input
              placeholder="请输入对应章节"
              v-model="postEssay.section"
              class="w150"
              clearable>
            </el-input>
          </li>
          ...
          <li v-if="optionValue == '问答题'">
            <span>难度等级:</span>
            <el-select v-model="postEssay.level" placeholder="选择难度等级" class="w150">
              <el-option
                v-for="item in levels"
                :key="item.value"
                :label="item.label"
          ...
          <li v-if="optionValue == '选择题'">
              <span>分值:</span>
              <a-input-number id="inputNumber" v-model:value="postChange.score" :min="1" :max="10" presion="0"/>
          </li>
          <li v-if="optionValue == '判断题'">
              <span>分值:</span>
              <a-input-number id="inputNumber" v-model:value="postJudge.score" :min="1" :max="10" presion="0"/>
          </li>
          <li v-if="optionValue == '填空题'">
              <span>分值:</span>
              <a-input-number id="inputNumber" v-model:value="postFill.score" :min="1" :max="10" presion="0"/>
          </li>
          <li v-if="optionValue == '问答题'">
              <span>分值:</span>
              <a-input-number id="inputNumber" v-model:value="postEssay.score" :min="1" :max="30" presion="0"/>
          </li>
        </ul>
        <!-- 选择题部分 -->
        ...
        <!-- 问答题 -->
        <div class="change fill" v-if="optionValue == '问答题'">
          <div class="title">
            <el-tag>题目:</el-tag><span>输入题目,形如--计算机网络系统的组成是？</span>
            <el-input
              type="textarea"
              rows="4"
              v-model="postEssay.question"
              placeholder="请输入题目内容"
              resize="none"
              class="answer">
            </el-input>
          ...
          <div class="fillAnswer">
            <el-tag>正确答案:</el-tag>
            <el-input v-model="postEssay.answer"></el-input>
          </div>
          <div class="title analysis">
            <el-tag type="success">解析:</el-tag><span>下方输入框中输入答案解析</span>
            <el-input
          ...
          <div class="submit">
            <el-button type="primary" @click="essaySubmit()">立即添加</el-button>
          </div>
        </div>
      </section>
    </el-tab-pane>
    <el-tab-pane name="second">
        <template v-slot:label>
            <span ><i class="iconfont icon-daoru-tianchong"></i>在线组卷</span>
        </template>
      <div class="box">
        <ul>
          <li>
          ...
          <li>
            <span>问答题数量：</span> <el-input type="text" v-model="essayNumber"></el-input>
          </li>
          <li>
...
export default {
  data() {
    return {
      ...
      postPaper: { //考试管理表对应字段
        paperId: null,
        questionType: null, // 试卷类型 1--选择题  2--填空题   3--判断题  4--问答题
        questionId: null,
      }
    };
  },
  ...
  methods: {
    ...
    changeSubmit() { //选择题题库提交
      this.postChange.subject = this.subject
        console.log("选择题数据",this.postChange)
      request({
          url: `/paper/addOneChoiceQuestion`,
          method: 'post',
          data: {
            paperId: this.paperId,
            question: this.postChange
          }
      }).then((res) => {
          if(res.data.success){
    ...
    fillSubmit() { //填空题提交
      this.postFill.subject = this.subject
        console.log("填空题数据",this.postFill)
      request({
          url: `/paper/addFillQuestion`,
          method: 'post',
          data: {
            paperId: this.paperId,
            question: this.postFill
          }
      }).then((res) => {
          if(res.data.success){
              ElMessage.success("已添加到题库");
    ...
    judgeSubmit() { //判断题提交
      this.postJudge.subject = this.subject
        console.log("判断题数据",this.postJudge)
       request({
          url: `/paper/addJudgeQuestion`,
          method: 'post',
          data: {
            paperId: this.paperId,
            question: this.postJudge
          }
      }).then((res) => {
          if(res.data.success){
              ElMessage.success("已添加到题库");
    ...
    essaySubmit(){ //问答题提交
        ...
        console.log("问答题数据",this.postEssay)
        request({
          url: `/paper/addEssayQuestion`,
          method: 'post',
          data: {
            paperId: this.paperId,
            question: this.postEssay
          }
      }).then((res) => {
          if(res.data.success){
              ElMessage.success("已添加到题库");
